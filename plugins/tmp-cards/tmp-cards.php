<?php
/**
 * Plugin Name: TMP Visitenkarten
 * Description: Digitale Mitarbeiter-Visitenkarten für This Moment Pictures mit VCard, QR-Code, PWA und Teilen-Funktion.
 * Version: 1.0.0
 * Author: Möller & Deiners GbR
...
final class MM_Digital_Cards {
    const POST_TYPE = 'tmp_digital_card';
    const VERSION = '1.0.0';
    const FIELDS = [
        ...
        'share_app' => 'WhatsApp-App zum Teilen', 'photo_url' => 'Porträtfoto URL (optional)',
        'extra_links' => 'Zusätzliche Links (je Zeile: Titel|URL)'
    ];
    const COMPANY_FIELDS = [
        'company' => 'Unternehmen', 'company_phone' => 'Telefon',
        'website' => 'Website', 'website_label' => 'Bezeichnung der Website',
        'logo_url' => 'Logo URL', 'welcome' => 'Begrüßungstext',
        'street' => 'Straße', 'zip' => 'PLZ', 'city' => 'Ort'
    ];
        ...
        if (!$self->card_by_slug('sebastian')) {
            ...
            if ($id && !is_wp_error($id)) {
                $seed = [
                    ...
                    'job_title'=>'Hochzeitsfotograf','phone'=>'','mobile'=>'0173 5848857',
                    'whatsapp'=>'0173 5848857','email'=>'info@thismomentpictures.de','share_app'=>'business',
                    'photo_url'=>'https://thismomentpictures.de/wp-content/uploads/2026/03/WhatsApp-Image-2025-10-06-at-21.34.32-1.jpg',
                    'extra_links'=>"Galerie|https://thismomentpictures.de/galerie/\nSo arbeite ich|https://thismomentpictures.de/meine-philosophie/\nPreise für Hochzeitsreportagen|https://thismomentpictures.de/was-kostet-ein-hochzeitsfotograf/\nHochzeit anfragen|https://thismomentpictures.de/kontakt/\nInstagram|https://www.instagram.com/thismomentpictures.de/"
                ];
    ...
    public function maybe_upgrade() {
        $logo_url = 'https://thismomentpictures.de/wp-content/uploads/LogosundCofuerPagebuild/weblogo1.png';
        $photo_url = 'https://thismomentpictures.de/wp-content/uploads/2026/03/WhatsApp-Image-2025-10-06-at-21.34.32-1.jpg';
        $current = get_option('mm_cards_company');
        if (!is_array($current) || !$current) $current = [
            'company'=>'This Moment Pictures', 'company_phone'=>'0173 5848857',
            'website'=>'https://thismomentpictures.de/', 'website_label'=>'This Moment Pictures',
            'logo_url'=>$logo_url,
            'welcome'=>'Hochzeitsfotograf für echte Momente.', 'street'=>'Bleichrasen 2',
            'zip'=>'99817', 'city'=>'Eisenach'
        ];
        if (empty($current['logo_url']) || str_contains($current['logo_url'], '/wp-content/plugins/tmp-cards/assets/')) $current['logo_url'] = $logo_url;
        ...
        foreach (get_posts(['post_type'=>self::POST_TYPE,'post_status'=>'any','numberposts'=>-1]) as $card) {
            ...
            $photo = get_post_meta($card->ID, '_mm_photo_url', true);
            if (str_contains((string)$photo, '/wp-content/plugins/tmp-cards/assets/')) update_post_meta($card->ID, '_mm_photo_url', $photo_url);
        }
    }

    public function company_menu() {
        add_submenu_page('edit.php?post_type='.self::POST_TYPE, 'TMP Visitenkarten Einstellungen', 'Einstellungen', 'manage_options', 'mm-cards-settings', [$this, 'company_settings_page']);
    }
    ...
    public function company_settings_page() {
        ...
        echo '<div class="wrap"><h1>TMP Visitenkarten – Unternehmensdaten</h1><p>Diese Angaben gelten zentral für alle digitalen Visitenkarten.</p><form method="post" action="options.php">';
    ...
    private function data($post) {
        ...
        if (!$d['photo'] && !empty($d['photo_url'])) $d['photo'] = $d['photo_url'];
        if (!$d['share_app']) $d['share_app'] = $d['slug'] === 'sebastian' ? 'business' : 'standard';
    ...
    private function manifest($slug) {
        ...
        echo wp_json_encode(['name'=>'Meine Visitenkarte – '.trim($d['first_name'].' '.$d['last_name']),'short_name'=>$d['first_name'] ?: 'Visitenkarte','start_url'=>$d['url'].'?owner=1','scope'=>$d['url'],'display'=>'standalone','background_color'=>'#15120f','theme_color'=>'#15120f','icons'=>$icon ? [['src'=>$icon,'sizes'=>'any','type'=>'image/png','purpose'=>'any']] : []], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE); exit;
    }
    ...
    private function render($d) {
        ...
        $share_title = $name . ' | This Moment Pictures';
...
?><!doctype html><html lang="de"><head><meta charset="<?php bloginfo('charset'); ?>"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#15120f"><meta name="robots" content="noindex,follow"><title><?php echo esc_html($name.' – '.$d['company']); ?></title><meta name="description" content="Digitale Visitenkarte von <?php echo esc_attr($name.', '.$d['job_title'].' bei '.$d['company']); ?>"><link rel="canonical" href="<?php echo esc_url($d['url']); ?>"><link rel="manifest" href="<?php echo esc_url(home_url('/?mm_card_manifest='.$d['slug'])); ?>"><link rel="apple-touch-icon" href="<?php echo esc_url($site_icon); ?>"><?php wp_head(); ?><link rel="stylesheet" href="<?php echo esc_url($assets.'card.css?ver='.self::VERSION); ?>"></head><body data-owner="<?php echo esc_attr($d['slug']); ?>" class="<?php echo $owner_view ? 'owner-view' : 'contact-view'; ?>"><main class="card"><section class="hero"><?php if(!empty($d['logo_url'])): ?><img class="brand-logo" src="<?php echo esc_url($d['logo_url']); ?>" alt="<?php echo esc_attr($d['company']); ?>"><?php else: ?><div class="brand">THIS MOMENT <span>PICTURES</span></div><?php endif; ?><?php if($d['photo']): ?><img class="portrait" src="<?php echo esc_url($d['photo']); ?>" alt="<?php echo esc_attr($name); ?>" width="180" height="180"><?php else: ?><div class="portrait placeholder" aria-hidden="true"><?php echo esc_html(mb_substr($d['first_name'],0,1).mb_substr($d['last_name'],0,1)); ?></div><?php endif; ?><h1><?php echo esc_html($name); ?></h1><?php if($d['job_title']): ?><p class="role"><?php echo esc_html($d['job_title']); ?></p><?php endif; ?><?php if($d['company']): ?><p class="company"><?php echo esc_html($d['company']); ?></p><?php endif; ?></section>
        ...
        <?php else: ?>
        <?php if(!empty($d['welcome'])): ?><p class="welcome"><?php echo esc_html($d['welcome']); ?></p><?php endif; ?>
        ...
        <section class="links">
        <?php if (!$owner_view): ?>
        <?php foreach(preg_split('/\r\n|\r|\n/', (string)$d['extra_links']) as $line): $parts=array_map('trim',explode('|',$line,2)); if(count($parts)!==2 || !$parts[0] || !filter_var($parts[1],FILTER_VALIDATE_URL)) continue; ?><a class="track" data-event="digital_card_link" href="<?php echo esc_url($parts[1]); ?>"><?php echo esc_html($parts[0]); ?> <span>›</span></a><?php endforeach; ?>
        <?php if($d['website']): ?><a class="track" data-event="digital_card_website" href="<?php echo esc_url($d['website']); ?>"><?php echo esc_html($d['website_label'] ?: $d['company']); ?> <span>›</span></a><?php endif; ?>
...
new BW_Digital_Cards_GitHub_Updater(__FILE__, MM_Digital_Cards::VERSION, 'tmp-cards', 'tmp-cards.zip');
}
