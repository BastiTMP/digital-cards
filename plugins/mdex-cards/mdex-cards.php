<?php
/**
 * Plugin Name: mdex Visitenkarten
 * Description: Digitale Mitarbeiter-Visitenkarten für mdexpertise mit VCard, QR-Code, PWA und Teilen-Funktion.
 * Version: 1.0.0
...
final class MM_Digital_Cards {
    const POST_TYPE = 'mdex_digital_card';
    const VERSION = '1.0.0';
    const FIELDS = [
        ...
        if (!$self->card_by_slug('sebastian')) {
            ...
            if ($id && !is_wp_error($id)) {
                $seed = [
                    ...
                    'job_title'=>'Kfz-Sachverständiger','phone'=>'','mobile'=>'0173 5848857',
                    'whatsapp'=>'0173 5848857','email'=>'info@mdexpertise.de','share_app'=>'business',
                    'photo_url'=>'https://mdexpertise.de/wp-content/uploads/2025/10/sebastian-moeller-ihr-kfz-gutachter-in-eisenach-768x997.jpg',
                    'extra_links'=>"Unfallgutachten|https://mdexpertise.de/unfallgutachten-eisenach/\nKostenlosen Unfallbericht herunterladen|https://mdexpertise.de/wp-content/uploads/2026/03/Europaeischer-Unfallbericht-MM-und-mdex.pdf\nKostenvoranschlag|https://mdexpertise.de/kostenvoranschlag-unfall-eisenach/\nBeweissicherungsgutachten|https://mdexpertise.de/beweissicherungsgutachten-eisenach/"
                ];
    ...
    public function maybe_upgrade() {
        $logo_url = 'https://mdexpertise.de/wp-content/uploads/2025/09/mdex_logo_black-trans.png';
        $photo_url = 'https://mdexpertise.de/wp-content/uploads/2025/10/sebastian-moeller-ihr-kfz-gutachter-in-eisenach-768x997.jpg';
        $current = get_option('mm_cards_company');
        if (!is_array($current) || !$current) $current = [
            'company'=>'mdexpertise', 'company_phone'=>'0173 5848857',
            'website'=>'https://mdexpertise.de/', 'website_label'=>'mdexpertise',
            'logo_url'=>$logo_url,
            'welcome'=>'Ihr unabhängiger Kfz-Sachverständiger in Eisenach.', 'street'=>'Bleichrasen 2',
            'zip'=>'99817', 'city'=>'Eisenach'
        ];
        if (empty($current['logo_url']) || str_contains($current['logo_url'], '/wp-content/plugins/mdex-cards/assets/')) $current['logo_url'] = $logo_url;
        ...
        foreach (get_posts(['post_type'=>self::POST_TYPE,'post_status'=>'any','numberposts'=>-1]) as $card) {
            ...
            if (str_contains((string)$photo, '/wp-content/plugins/mdex-cards/assets/')) update_post_meta($card->ID, '_mm_photo_url', $photo_url);
        }
    }

    public function company_menu() {
        add_submenu_page('edit.php?post_type='.self::POST_TYPE, 'mdex Visitenkarten Einstellungen', 'Einstellungen', 'manage_options', 'mm-cards-settings', [$this, 'company_settings_page']);
    }
    ...
    public function company_settings_page() {
        ...
        echo '<div class="wrap"><h1>mdex Visitenkarten – Unternehmensdaten</h1><p>Diese Angaben gelten zentral für alle digitalen Visitenkarten.</p><form method="post" action="options.php">';
...
new BW_Digital_Cards_GitHub_Updater(__FILE__, MM_Digital_Cards::VERSION, 'mdex-cards', 'mdex-cards.zip');
}
